...
var concatStream = __importStar(require("concat-stream"));
...
var debug = debug_1["default"]('node-soap');
...
var Client = /** @class */ (function (_super) {
    ...
    function Client(wsdl, endpoint, options) {
        ...
        if (options.overridePromiseSuffix) {
            _this.overridePromiseSuffix = options.overridePromiseSuffix;
        }
    ...
    Client.prototype._initializeServices = function (endpoint) {
        ...
        for (var name in services) {
            var service = this._defineService(services[name], endpoint);
            this[name] = service;
        }
    };
    ...
    Client.prototype._definePort = function (port, endpoint) {
        ...
        for (var name in methods) {
            ...
            this[methodName + (this.overridePromiseSuffix || 'Async')] = bluebird_1["default"].promisify(method, { multiArgs: true });
        }
        return def;
    };
    ...
    Client.prototype._invoke = function (method, args, location, callback, options, extraHeaders) {
        ...
        if ((style === 'rpc') && ((!input || input.parts || input.name === 'element') || args === null)) {
            assert.ok(!style || style === 'rpc', 'invalid message definition for document style binding');
            message = this.wsdl.objectToRpcXML(name, args, alias, ns, (input && input.name !== 'element'));
